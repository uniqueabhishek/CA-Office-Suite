...
class TestMergeFilesLogic:
    ...
    def test_every_file_unreadable_raises_a_clear_error(self, tmp_path):
        """Falling through would build a sheetless workbook and fail obscurely."""
        broken = os.path.join(tmp_path, "broken.xlsx")
        with open(broken, "w", encoding="utf-8") as fh:
            fh.write("this is not a workbook")
