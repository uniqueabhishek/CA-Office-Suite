...
class TestSplitFilesToZip:
    ...
    def test_colliding_names_are_de_duplicated(self, tmp_path):
        ...
        entries = zipfile.ZipFile(utils.split_files_to_zip(paths, ["report.xlsx", "report.xlsx"])).namelist()
    ...
    def test_unsafe_sheet_name_is_sanitised(self, tmp_path):
        ...
        entries = zipfile.ZipFile(utils.split_files_to_zip([p], ["a/b\\c.csv"])).namelist()
...
class TestProcessExcelFile:
    def test_applies_transformations_and_names_the_output(self, tmp_path):
        ...
        stream, name = utils.process_excel_file(p, "in.xlsx", {"trim": True, "text_case": "upper"})
        assert name == "in_processed.xlsx"
        assert load_workbook(stream).active["A2"].value == "ALICE"

    def test_number_formats_do_not_leak_between_sheets(self, tmp_path):
        """
        Regression: the per-sheet number format maps used to be merged into one
        dict and applied to every worksheet, so a text column sharing a header
        name with a numeric one elsewhere was formatted as a number.
        """
        p = os.path.join(tmp_path, "two.xlsx")
        with pd.ExcelWriter(p, engine="openpyxl") as writer:
            pd.DataFrame({"Amount": ["10", "20"]}).to_excel(writer, sheet_name="Numeric", index=False)
            pd.DataFrame({"Amount": ["n/a", "pending"]}).to_excel(writer, sheet_name="Textual", index=False)

        stream, _ = utils.process_excel_file(
            p, "two.xlsx", {"number_format": True, "number_format_option": "2_decimals"}
        )
        wb = load_workbook(stream)

        assert wb["Numeric"]["A2"].number_format == "#,##0.00"
        assert wb["Textual"]["A2"].number_format == "General"

    def test_long_sheet_names_are_truncated(self, tmp_path):
        p = os.path.join(tmp_path, "long.xlsx")
        with pd.ExcelWriter(p, engine="openpyxl") as writer:
            pd.DataFrame({"A": [1]}).to_excel(writer, sheet_name="a" * 31, index=False)
